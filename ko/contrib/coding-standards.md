...
예를 들어 union type은 PHP 8.0 이상에서만 지원되므로, 코어에서는 사용할 수 없습니다.
...
사용자가 소스를 수정하더라도 별도의 프로그램이나 개발 환경을 이용한 압축, 컴파일, 빌드 등의 과정을 거치지 않고 새로고침 후 곧바로 사용할 수 있어야 한다는 뜻입니다.
단, 원본 소스가 공개되어 있는 외부 라이브러리를 끌어다 사용하는 경우에는 압축된 상태로 제공할 수 있습니다.

composer 라이브러리를 업데이트할 때는 라이믹스에서 공식적으로 지원하는 환경(PHP 7.4 이상)에서 정상 작동하는 버전으로 고정시켜야 합니다.
