...
가장 큰 차이점으로, v1 문법을 전혀 사용하지 않고 100% Blade 스타일로 템플릿을 작성하는 것도 가능합니다.
조건문, 루프문 등의 좌우에 `<!--` `-->` 주석을 붙일 필요가 없고,
아래에서 소개하는 그 밖의 모든 기능도 `@지시자`의 형태로 사용할 수 있습니다.

단, v1 문법처럼 조건문 좌우에 주석을 붙이거나, 중괄호를 하나만 쓰더라도 정상 작동합니다.
...
XE 1.4.4 버전 이후 수많은 템플릿 해석 오류의 원인이 된 `<block>` 가상 태그, `loop` 속성, `cond` 속성을
더이상 지원하지 않습니다.

하나의 속성을 표시할지 숨길지 결정하는 `|cond=""` 문법은 계속 지원하지만,
더욱 강력한 Blade 스타일의 `@class`, `@style`, `@selected` 등의 지시자를 활용할 것을 추천합니다.
...
코어에서 통합 관리하는 리소스 로딩 기능, 상대경로 자동 변환,
출력 필터 등 v1의 핵심 기능들도 대부분 그대로 가져왔습니다.
...
Laravel의 코드를 그대로 사용하기에는 어려움이 있어,
**상속 및 컴포넌트 기능을 제외한 Blade 스타일의 문법만 차용하고** 템플릿 컴파일러는 자체 구현했습니다.
...
단, v1 문법처럼 중괄호를 하나만 쓰는 것도 허용합니다. 이 때, 중괄호 바로 안쪽에 공백이 있어서는 안 됩니다.
...
- 변수 앞에 `\`를 붙여 이스케이프 처리한 경우 (예: `\$foo`) 로컬 변수가 됩니다.
...
(HTML에서 널리 사용하는 속성에 다른 의미를 부여하지 말자는 취지로, `target`은 더이상 권장하지 않기로 했습니다.)
...
그 중 2가지 조건은 대체 문법으로도 구현할 수 있습니다. v1 문법의 `cond`에 해당합니다.
...
> [주의]
> Laravel Blade에서는 인클루드시 변수를 전달하면 부모 템플릿이 가진 변수들도 상속받습니다.
> 그러나 라이믹스의 템플릿은 자체적으로 변수를 갖고 있지 않고 `Context`를 공유하므로,
> Laravel Blade와 작동 방식이 다른 점 유의하시기 바랍니다.

대체 문법에서도 `vars` 속성을 사용하여 변수를 전달할 수 있고, 조건부 인클루드와 함께 쓸 수도 있습니다.
...
파라미터의 순서는 파일명, media, 로딩 순서, 변수 순이며, 파일명 외에는 모두 생략할 수 있습니다.
단, media와 로딩 순서는 각각 문자열과 정수로 전달해야 하며 변수를 사용할 수 없습니다.

v1과 유사한 대체 문법을 사용하면 각 파라미터의 역할을 더 분명하게 표현할 수 있습니다.
`src` 속성은 `target`으로 써도 정상 인식합니다.
표현상의 분명한 이점이 있으므로, 대체 문법의 사용을 피하라고 권하지는 않겠습니다.
...
리소스 언로딩을 위한 지시자는 없습니다. 아래에서 설명할 PHP 코드 직접 실행 문법을 사용하여
`Context::unloadFile()` 메소드를 호출하면 됩니다.
...
`@lang`을 사용하여 언어 파일의 내용을 쉽게 불러올 수 있습니다.
...
이 지시자들에는 다양한 변형이 있으며, Laravel Blade와 동일하게 작동합니다.