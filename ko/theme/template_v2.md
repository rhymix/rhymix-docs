...
이렇게 v1과 비슷한 점이 많아 적응하기 쉽다는 것은
라이믹스의 차기 템플릿 문법으로 Blade를 채택한 이유 중 하나입니다.
그러나 v1 호환 문법은 어디까지나 v1에 익숙한 테마 제작자와 라이믹스 사용자들이
기존 템플릿을 변환하고 새 템플릿 문법을 배우는 데 도움을 주기 위해 지원하는 것이므로,
가능하면 v2 정규 문법만을 사용할 것을 권장합니다.
...
한 쌍의 중괄호만 사용할 때는 클로져 등 중괄호가 포함되는 코드를 사용할 수 없고,
중괄호 안에서 줄바꿈을 할 수 없다는 제한이 있습니다.

v1 호환 문법은 일반적인 CSS, JS 문법과 충돌하는 경우가 많으므로,
라이믹스 2.1.22부터 CSS, JS 문맥에서는 이 문법을 허용하지 않습니다.
아래의 위치에서는 반드시 두 쌍의 중괄호로 데이터를 출력해야 합니다.
...
@desktop
    PC입니다.
	아래 @mobile @endmobile 지시자의 반대 조건입니다.
@enddesktop
```
...
@mobile
    모바일입니다.
	라이믹스 2.1.21 이전: 모바일 뷰 설정과 m 파라미터에 영향을 받습니다.
	라이믹스 2.1.22 이후: 접속한 User-Agent와 "태블릿도 모바일 취급" 설정만을 따릅니다.
@endmobile
```
...
PHP 코드 내에서 함수를 선언하거나 클로져를 사용할 경우,
함수의 파라미터나 함수 내에서 사용하는 변수를 템플릿 변수로 해석하여 오류가 발생할 수 있습니다.
이를 방지하기 위해 변수 앞에 `\` 문자를 붙여 로컬 변수임을 명시하거나,
함수 선언 부분을 `@verbatim`으로 묶어 해석을 방지해야 합니다.

어떤 방법을 사용하는 것이 더 나은지는 상황에 따라 다릅니다.
예를 들어 아래의 예제에서는 어떤 변수도 해석하지 않으므로,
해당 코드보다 윗부분에서 선언한 `$source` 변수를 참조할 수 없고,
해당 코드에서 선언한 `$result` 변수를 아래에서 다시 참조할 수도 없습니다.
...
별도의 PHP 파일(컨트롤러 또는 모델 클래스 등)에 코드를 작성한 후 인클루드하는 것을 권장합니다.
...
`@widget`을 사용하면 현재 위치에 위젯을 삽입할 수 있습니다.
예전 방식의 위젯 태그를 사용하는 것보다 간결하고, 위젯에 전달할 변수를 커스터마이징하기도 편리합니다.

    @widget('content', $args)

위젯에 전달하는 `$args`는 배열이나 오브젝트여야 하고, `skin` 등 해당 위젯을 렌더링하는 데 필요한
변수들을 포함하고 있어야 합니다.
...
이런 특성을 고려할 때 Laravel Blade의 컴포넌트 설계는 라이믹스와 맞지 않다고 판단하였으며,
적절한 대안을 마련할 때까지 컴포넌트 관련 기능의 적용은 보류합니다.

단, [인클루드시 변수 전달](#인클루드시_변수_전달) 기능을 활용하여
상당히 독립적인 컴포넌트와 같은 템플릿을 구현할 수 있습니다.
클래스를 선언하여 사용한다는 점에서 위젯의 역할과도 겹치는 면이 있으므로,
복잡한 기능이 필요한 컴포넌트는 위젯을 활용하는 것을 권장합니다.
